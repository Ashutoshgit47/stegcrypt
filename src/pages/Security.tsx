...
const guarantees = [
  {
    icon: Server,
    title: 'No Backend',
    description: 'StegCrypt operates without a server. There are no uploads, API calls, or analytics tracking your data.',
    status: 'guaranteed',
  },
  {
    icon: Lock,
              ...
              {cryptoDetails.map((item) => (
                <div key={item.label} className="py-3 first:pt-0 last:pb-0">
                  <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-1 sm:gap-4">
                    ...
                    <span className="text-sm font-mono font-medium text-foreground sm:text-right">{item.value}</span>
                  </div>
                </div>
              ))}
            </div>
